...
import { SimulcastPage } from './components/SimulcastPage';
import { ComingSoonPage } from './components/ComingSoonPage';
import { NewsPage } from './components/NewsPage';
import { NotificationsPage } from './components/NotificationsPage';
import { SearchModal } from './components/SearchModal';
import { CAROUSEL_ITEMS, NEW_RELEASES, POPULAR_ANIME } from './constants';
import { AnimeDetails } from './types';
import { AnimatePresence, motion } from 'framer-motion';
import { BookOpen, Gamepad2, ShoppingBag } from 'lucide-react';

type ViewState = 'discover' | 'series' | 'new' | 'popular' | 'simulcast' | 'manga' | 'games' | 'shop' | 'news' | 'notifications';
...
const App: React.FC = () => {
  ...
  const handleWatch = (anime: AnimeDetails) => {
    setSelectedAnime(anime);
    setView('series');
    window.scrollTo({ top: 0, behavior: 'smooth' });
  };

  const handleSearchSelection = (anime: AnimeDetails) => {
    handleWatch(anime);
  ...
  const handleNavigate = (target: string) => {
    const validTargets: ViewState[] = ['discover', 'series', 'new', 'popular', 'simulcast', 'manga', 'games', 'shop', 'news', 'notifications'];
    if (validTargets.includes(target as ViewState)) {
  ...
  return (
    <div className="min-h-screen bg-brand-black text-white selection:bg-brand-orange selection:text-black font-sans">
      <Navbar 
        ...
        <AnimatePresence mode="wait">
          {view === 'discover' && (
            <React.Fragment key="discover">
              <HeroCarousel items={CAROUSEL_ITEMS} onWatch={handleWatch} />
              <Discover onWatch={handleWatch} />
            </React.Fragment>
          )}
          ...
          {view === 'new' && (
            ...
            >
              <GridPage 
                title="New Releases" 
                description="Fresh from Japan. The latest episodes and new series premieres available right now."
                items={NEW_RELEASES}
                onWatch={handleWatch}
              />
            </motion.div>
          ...
          {view === 'popular' && (
            ...
            >
              <GridPage 
                title="Most Popular" 
                description="The most watched anime on AniStream right now. Don't miss out on the hype."
                items={POPULAR_ANIME}
                onWatch={handleWatch}
              />
            </motion.div>
          ...
          {view === 'simulcast' && (
             <motion.div
              ...
              exit={{ opacity: 0 }}
            >
              <SimulcastPage onWatch={handleWatch} />
            </motion.div>
          )}
          ...
          {view === 'manga' && (
             <motion.div
              ...
              exit={{ opacity: 0 }}
            >
              <ComingSoonPage title="Manga Reader" icon={BookOpen} />
            </motion.div>
          )}
          ...
          {view === 'games' && (
             <motion.div
              ...
              exit={{ opacity: 0 }}
            >
              <ComingSoonPage title="AniStream Games" icon={Gamepad2} />
            </motion.div>
          )}
          ...
          {view === 'shop' && (
             <motion.div
              ...
              exit={{ opacity: 0 }}
            >
              <ComingSoonPage title="Merch Shop" icon={ShoppingBag} />
            </motion.div>
          )}
          ...
          {view === 'series' && selectedAnime && (
            <React.Fragment key="series">
               <SeriesPage anime={selectedAnime} />
            </React.Fragment>
          )}
        </AnimatePresence>
      </main>
      ...
      <footer className="border-t border-gray-900 bg-brand-black py-12 px-8 md:px-16 relative z-20">
        <div className="flex flex-col md:flex-row justify-between items-center gap-6">
           ...
           <div className="flex gap-6 text-sm text-gray-500">
             ...
             <a href="#" className="hover:text-brand-orange transition-colors">Cookie Preferences</a>
           </div>
